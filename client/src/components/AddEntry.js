...
export function AddEntry(props) {
  ...
  const handleSubmit = async (evt) => {
    ...
    const response = await axios.post((url = url), data, { headers }).catch(function (error) {
      outputData = {
        isError: true,
        error: error.message,
        message: ""
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        ...
        <Grid item>
          <Button id="add_btn" type="submit" variant="contained" color="primary" className={classes.button}>
            Add
          </Button>
          <Button
            id="gosearch_btn"
